...
# BOJ commands
BOJ = discord.SlashCommandGroup(name='boj', description='BOJ commands', guild_ids=config['guild_ids'].values())
tier_names = ['Unrated', 'Bronze', 'Silver', 'Gold', 'Platinum', 'Diamond', 'Ruby']
tier_counts = ['V', 'IV', 'III', 'II', 'I']
tier_colors = [0x302C2C, 0xA85C1C, 0x4C5C7C, 0xE49C34, 0x6CDCAC, 0x54B4F4, 0xEC3464]
...
try:
	...
	async def problem_search(ctx, problem_id: int):
		...
		try:
			objects = loads(BeautifulSoup(urlopen(url).read(), 'html.parser').get_text())
		except HTTPError:
		...
		problem = {
			"id": objects['problemId'],
			"title": objects['titleKo'],
			"tier": tier_names[0] if objects['level'] == 0
			else tier_names[(objects['level'] - 1) // 5 + 1] + ' ' + tier_counts[(objects['level'] - 1) % 5],
			"tier_icon": f"https://static.solved.ac/tier_small/{objects['level']}.svg",
			"tier_color": tier_colors[0] if objects['level'] == 0
			else tier_colors[(objects['level'] - 1) // 5 + 1],
	...
	async def user_search(ctx, user_name: str):
		...
		try:
			objects = loads(BeautifulSoup(urlopen(url).read(), 'html.parser').get_text())['items'][0]
		except IndexError:
		...
		user = {
			"name": objects['handle'],
			"bio": objects['bio'],
			"organizations": [x['name'] for x in objects['organizations']],
			"profile_image": objects['profileImageUrl'],
			"solved_count": objects['solvedCount'],
			"rating": objects['rating'],
			"tier": 'Master' if objects['tier'] == 31
			else tier_names[(objects['tier'] - 1) // 5 + 1] + ' ' + tier_counts[(objects['tier'] - 1) % 5],
			"tier_color": random.choice([0xd989c1, 0xa0e4fc, 0xa6c0fb, 0xe685b5, 0xaca0fb, 0xb494f4]) if objects[
				                                                                                             'tier'] == 31
			else tier_colors[(objects['tier'] - 1) // 5 + 1],
			"rank": objects['rank']
		}
